        ...
        if event.type == 'MARKET':
            for symbol in self.symbol_list:
                ...
                if data is not None and len(data) >= self.long_period:
                    ...
                    price = df['Close'][-1]
                    self.strategy[symbol] = self.strategy[symbol]._append({'Date': date, 'Short': price_short, 'Long': price_long}, ignore_index=True)
                    if self.bought[symbol] == False and price_short > price_long:
                        ...
                        self.signals[symbol] = self.signals[symbol]._append({'Signal': quantity, 'Date': date}, ignore_index=True)
